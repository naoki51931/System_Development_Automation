...
The measured staging fixed run rate is now about $210/month, dominated by seven Interface Endpoints across two AZs (~$127.75), RDS (~$54.9), and ALB/public IPv4 (~$26.62). `staging_mode="idle"` targets $2–4/month by omitting those resources while preserving state/data/control-plane foundations. Seven endpoints are more expensive than one NAT Gateway in this region; ACTIVE should prefer a separately reviewed single-NAT private-task design, or consider public-IP tasks only for short synthetic tests after security review. The current known-working ACTIVE mode retains endpoints until that separate network plan is approved. See `staging_idle_mode.md`.

RDS idle removal is not ready merely because Terraform can show a destroy. It requires an available manual snapshot and recorded restore test, zero runtime/migration activity and safe connections, a separate deletion-protection change, fresh approved plan, and human approval. As of 2026-08-10, `RDS_IDLE_REMOVAL = BLOCKED` because no manual snapshot exists.
...
The monitoring input decision is complete: ignored `staging-prerequisites/terraform.tfvars` sets `info@nagi-neco.com` and a 150 USD monthly Budget. The former 100 GBP setting was rejected because AWS Budgets for this account accepts USD only. CloudWatch ALB/ECS/worker/RDS alarms in main staging publish to the prerequisite `system-navigator-staging-alerts`; the email subscription remains `PendingConfirmation` until confirmed by a human. AWS Budgets sends actual 50/80/100% and forecasted 100% notices directly to the same address.

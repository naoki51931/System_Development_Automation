...
The root now adds an outer cost lifecycle: `staging_mode="active"` keeps the phase model above, while `staging_mode="idle"` removes only the costly/ephemeral layer. IDLE retains VPC/subnets/route tables/Internet Gateway/task and DB security groups/DB subnet group, S3 Gateway Endpoint, artifact bucket with `prevent_destroy`, application Secret container, IAM and log groups. ECR/deploy role/SNS/Budget remain in the prerequisites state. IDLE omits RDS, ALB/listeners/target groups, seven Interface Endpoints, ECS cluster/task definitions/services, Cloud Map and infrastructure alarms. Production root/modules/state have no reference to this variable.

RDS accepts `snapshot_identifier` only through the mutually validated `restore_db_from_snapshot`/`db_snapshot_identifier` inputs. A snapshot ID is runtime operational data and must remain outside Git. Removing Terraform `prevent_destroy` from staging RDS makes an IDLE plan representable, but does not authorize it: deletion protection, manual snapshot lookup, runtime/connection prechecks, a plan-only blocker, saved-plan review and explicit human approval remain mandatory. See `staging_idle_mode.md`.
...
`environment/staging-prerequisites` is an independently reviewed state owning the two ECR repositories, GitHub staging deploy role/policy, staging SNS subscription, and 150 USD monthly Budget. The former 100 GBP setting was rejected by the AWS Budgets API and replaced. Conditional ACM and validation resources have count zero while custom domains are disabled. Its key is `system-navigator/staging/prerequisites.tfstate`; it contains no VPC, subnet, endpoint, RDS, ECS, ALB, artifact S3, or Secret. After approved push, main staging consumes full ECR `repository@sha256:...` values; backend and worker share a digest.

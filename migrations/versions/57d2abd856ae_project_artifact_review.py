...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "current_phase IN ('hearing', 'estimate', 'requirements', 'basic_design', 'detailed_design', 'development', 'testing', 'staging', 'acceptance', 'production', 'maintenance')",
            name="ck_projects_phase",
        ),
        sa.CheckConstraint(
            "status IN ('draft', 'hearing', 'estimating', 'awaiting_payment', 'requirements', 'basic_design', 'detailed_design', 'development', 'testing', 'staging', 'awaiting_acceptance', 'production', 'maintenance', 'suspended', 'closed')",
        ...
        sa.ForeignKeyConstraint(
            ["project_manager_user_id"], ["users.id"], ondelete="RESTRICT"
        ),
    ...
    op.create_index(
        "ix_projects_organization_status",
        "projects",
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "provider IN ('openai', 'anthropic')", name="ck_ai_runs_provider"
        ),
        sa.CheckConstraint(
    ...
    op.create_index(
        "ix_ai_runs_organization_status",
        "ai_runs",
        ["organization_id", "status"],
        unique=False,
    )
    op.create_index(
        "ix_ai_runs_project_created",
        "ai_runs",
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "artifact_type IN ('hearing_sheet', 'estimate', 'requirements_definition', 'basic_design', 'detailed_design', 'screen_design', 'api_specification', 'database_design', 'test_specification', 'test_report', 'operation_manual', 'release_procedure', 'source_code')",
            name="ck_artifacts_type",
        ),
        sa.CheckConstraint(
            "status IN ('draft', 'ai_reviewing', 'human_reviewing', 'revision_requested', 'approved', 'superseded')",
    ...
    op.create_index(
        "ix_artifacts_organization_type",
        "artifacts",
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "project_role IN ('project_owner', 'project_manager', 'reviewer', 'developer', 'customer', 'viewer')",
    ...
    op.create_index(
        "ix_approval_events_project_created",
        "approval_events",
        ["project_id", "created_at"],
        unique=False,
    )
    op.create_index(
        "ix_approval_events_version_created",
        "approval_events",
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "review_type IN ('ai', 'human', 'customer_acceptance', 'security', 'architecture', 'code_quality', 'test_quality')",
            name="ck_reviews_type",
        ),
        sa.CheckConstraint(
            "status IN ('pending', 'in_progress', 'passed', 'failed', 'changes_requested', 'cancelled')",
    ...
    op.create_index(
        "ix_reviews_project_created",
        "reviews",
        ["project_id", "created_at"],
        unique=False,
    )
    op.create_index(
        "ix_reviews_version_status",
        "reviews",
    ...
    op.create_index(
        "ix_review_comments_review_status",
        "review_comments",
        ["review_id", "status"],
        unique=False,
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_artifact_versions_artifact_created", table_name="artifact_versions"
    )

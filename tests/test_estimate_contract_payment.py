...
from app.api.pagination import decode_cursor, encode_cursor, paginate_query
...
def context(session: Session, suffix: str = "billing"):
    ...
    access = type(raw)(
        raw.user, raw.membership, frozenset({"organization_admin", "customer"})
    )
    project = create_project(
        session,
        access,
        project_code=f"P-{uuid.uuid4().hex[:12]}",
...
def estimate_fixture(
    session: Session, suffix: str = "estimate", valid_until: date | None = None
):
    ...
    estimate = create_estimate(
        ...
        project,
        access,
        estimate_number=f"E-{uuid.uuid4().hex[:12]}",
...
def test_estimate_cursor_pagination_is_stable_and_tenant_scoped(db_session: Session):
    organization, _user, access, project = context(db_session, "page")
    estimates = [
        create_estimate(
            db_session,
            project,
            access,
            estimate_number=f"PAGE-{index}",
    ...
    _other_org, _other_user, other_access, other_project = context(
        db_session, "page-other"
    )
    create_estimate(
        db_session,
        other_project,
        other_access,
        estimate_number="OTHER-PAGE",
        valid_until=date.today() + timedelta(days=30),
        include_ai_runs=False,
        include_artifacts=False,
        items=[],
    ...
    for estimate in estimates:
        estimate.created_at = shared_created_at
    db_session.flush()

    statement = select(Estimate).where(
        Estimate.organization_id == organization.id,
        Estimate.project_id == project.id,
    )
    scope = f"estimates:{organization.id}:{project.id}"
    first = paginate_query(
        db_session,
        statement,
        Estimate,
        None,
        2,
        lambda item: {"id": str(item.id)},
        scope=scope,
    )
    second = paginate_query(
        db_session,
        statement,
        Estimate,
        first["next_cursor"],
...
def test_cursor_rejects_expiry_scope_tampering_and_invalid_encodings():
    issued_at = datetime(2026, 1, 1, tzinfo=timezone.utc)
    cursor = encode_cursor(issued_at, uuid.uuid4(), scope="tenant-a", now=issued_at)
    assert decode_cursor(cursor, expected_scope="tenant-a", now=issued_at)

    for invalid in (cursor[:-2] + "xx", "not-base64!", "e30"):
        with pytest.raises(HTTPException):
            decode_cursor(invalid, expected_scope="tenant-a", now=issued_at)
    with pytest.raises(HTTPException):
        decode_cursor(cursor, expected_scope="tenant-b", now=issued_at)
    with pytest.raises(HTTPException):
        decode_cursor(
            cursor, expected_scope="tenant-a", now=issued_at + timedelta(hours=2)
        )


def active_contract_fixture(
    session: Session, suffix: str = "contract", contract_type: str = "development"
):
    ...
    contract = create_contract(
        ...
        project,
        access,
        contract_number=f"C-{uuid.uuid4().hex[:12]}",
...
@pytest.mark.parametrize(
    ...
    [
        ("option", Decimal("1"), Decimal("-1")),
        ("discount", Decimal("1"), Decimal("1")),
        ("option", Decimal("-1"), Decimal("1")),
    ],
)
def test_estimate_rejects_invalid_negative_values(
    item_type: str, quantity: Decimal, price: Decimal
):
...
def test_contract_requires_scope_and_both_acceptances(db_session: Session):
    ...
    transition_estimate(db_session, estimate, access, "submit", estimate.version)
    transition_estimate(db_session, estimate, access, "approve", estimate.version)
    other_org, _other_user, other_access, other_project = context(
        db_session, "other-contract"
    )
    with pytest.raises(HTTPException):
        create_contract(
            db_session,
...
@pytest.mark.parametrize(
    ("outcome", "expected_status", "project_status"),
    [
        ("succeeded", "succeeded", "requirements"),
        ("failed", "failed", "draft"),
        ("processing", "processing", "draft"),
    ],
)
def test_mock_payment_outcomes(
    db_session: Session, outcome: str, expected_status: str, project_status: str
):
    _org, _user, access, project, estimate, contract = active_contract_fixture(
        db_session, f"pay-{outcome}"
    )
    ...
    confirm_contract_payment(
        db_session, intent, contract, project, access, provider, intent.version
    )
...
def test_payment_amount_idempotency_duplicate_and_reconfirm(db_session: Session):
    ...
    with pytest.raises(AppError) as mismatch:
        create_contract_payment_intent(
            ...
            provider,
            idempotency_key=key,
            expected_amount=estimate.total_amount + Decimal("1"),
        )
    assert mismatch.value.code == "AMOUNT_MISMATCH"
    intent = create_contract_payment_intent(
        db_session, contract, access, provider, idempotency_key=key
    )
    assert (
        create_contract_payment_intent(
            db_session, contract, access, provider, idempotency_key=key
...
def test_provider_unavailable_maps_to_safe_error(db_session: Session):
    _org, _user, access, _project, _estimate, contract = active_contract_fixture(
        db_session, "provider-down"
    )
    with pytest.raises(AppError) as error:
        create_contract_payment_intent(
            db_session,
...
def maintenance_fixture(session: Session):
    organization, user, access, project, _estimate, contract = active_contract_fixture(
        session, "maintenance", contract_type="maintenance"
    )
    project.status = "production"
    project.current_phase = "production"
    plan = MaintenancePlan(
        name="Standard",
        code=f"standard-{uuid.uuid4().hex[:8]}",
        status="active",
        currency="JPY",
        monthly_price=Decimal("10000"),
        included_ai_minutes=60,
        included_human_minutes=30,
    ...
    maintenance = create_maintenance_contract(
        session, project, contract, plan, access, now
    )
...
def test_maintenance_delinquency_boundaries_and_no_aws_execution(db_session: Session):
    _org, _user, _access, _project, _contract, maintenance, start = maintenance_fixture(
        db_session
    )
    ...
    assert (
        advance_maintenance_delinquency(
            db_session, maintenance, start + timedelta(days=29, hours=23)
        )
        is None
    )
    assert maintenance.status == "past_due"
    assert (
        advance_maintenance_delinquency(
            db_session, maintenance, start + timedelta(days=30)
        )
        is None
    )
    ...
    assert (
        advance_maintenance_delinquency(
            db_session, maintenance, start + timedelta(days=60)
        )
        is None
    )
    assert maintenance.status == "suspended"
    request = advance_maintenance_delinquency(
        db_session, maintenance, start + timedelta(days=90)
    )
...
def test_deletion_requires_two_distinct_approvers(db_session: Session):
    organization, user, access, _project, _contract, maintenance, start = (
        maintenance_fixture(db_session)
    )
    mark_maintenance_payment_failed(db_session, maintenance, start)
    request = advance_maintenance_delinquency(
        db_session, maintenance, start + timedelta(days=90)
    ...
    approve_deletion_request(
        db_session, request, access, start + timedelta(days=90), request.version
    )
    ...
    second = User(
        cognito_sub=f"second-{uuid.uuid4()}",
        email=f"{uuid.uuid4()}@example.com",
        display_name="Second",
        status="active",
    )
    role = Role(
        code=f"second-{uuid.uuid4().hex[:20]}",
        display_name="Second admin",
        is_system=False,
    )
    membership = OrganizationMembership(
        organization_id=organization.id, user=second, status="active"
    )
    membership.roles.append(MembershipRole(role=role))
    db_session.add(membership)
    db_session.flush()
    raw = require_organization_access(
        organization.id, AuthenticatedUser(second), db_session
    )
    second_access = type(raw)(
        raw.user, raw.membership, frozenset({"organization_admin"})
    )
    approve_deletion_request(
        db_session, request, second_access, start + timedelta(days=90), request.version
    )
...
def test_mock_webhook_signature_hash_and_idempotency(db_session: Session):
    ...
    event = process_mock_webhook(
        db_session, provider, organization.id, payload, provider.sign_webhook(payload)
    )
...
def test_webhook_out_of_order_does_not_regress_succeeded_payment(db_session: Session):
    organization, _user, access, project, _estimate, contract = active_contract_fixture(
        db_session, "webhook-order"
    )
    provider = MockPaymentProvider()
    intent = create_contract_payment_intent(
        db_session, contract, access, provider, idempotency_key=f"idem-{uuid.uuid4()}"
    )
    ...
    process_mock_webhook(
        ...
        organization.id,
        succeeded,
        provider.sign_webhook(succeeded),
    )
    process_mock_webhook(
        db_session,
        provider,
        organization.id,
        failed_late,
        provider.sign_webhook(failed_late),
    )

...
def client_for(session: Session, user: User, token: str = "valid") -> TestClient:
    app = create_app()
    app.state.session_factory = sessionmaker(bind=session.get_bind(), expire_on_commit=False)
...
def test_project_api_rejects_unrelated_tenant_and_unaffiliated_creator(db_session: Session):
    allowed = Organization(name="Allowed API", status="active")
    denied = Organization(name="Denied API", status="active")
    member = User(cognito_sub="api-member", email="api-member@example.com", display_name="Member", status="active")
    outsider = User(cognito_sub="api-outsider", email="api-outsider@example.com", display_name="Outsider", status="active")
    role = Role(code="organization_admin", display_name="Admin", is_system=True)
    membership = OrganizationMembership(organization=allowed, user=member, status="active")
    ...
    created = member_client.post(
        ...
        json={"organization_id": str(allowed.id), "project_code": "API-1", "name": "API Project"},
    )
    ...
    assert outsider_client.post(
        "/api/v1/projects",
        headers={"Authorization": "Bearer valid"},
        json={"organization_id": str(denied.id), "project_code": "NO", "name": "Denied"},
    ).status_code == 403

...
def test_jwt_and_cookie_signature_tampering_rejected():
    provider=LocalAuthProvider("quality-secret");token=provider.issue("user")
    with pytest.raises(TokenVerificationError):provider.verify(token[:-2]+"xx")

def test_cursor_tampering_and_sql_payload_rejected():
    value=encode_cursor(__import__("datetime").datetime.now(__import__("datetime").timezone.utc),uuid.uuid4())
    with pytest.raises(Exception):decode_cursor(value+"' OR 1=1--")

def test_path_traversal_mime_and_large_file(tmp_path):
    storage=LocalArtifactStorage(tmp_path,max_file_size=4)
    with pytest.raises(AppError):storage.put_object("../secret",b"x","text/plain")
    with pytest.raises(AppError):storage.put_object("organizations/x/a",b"x","image/svg+xml")
    with pytest.raises(AppError):storage.put_object("organizations/x/a",b"12345","text/plain")

...
Staging also supports a low-cost `staging_mode="idle"`. It preserves remote state, VPC/subnets/routes/security groups, the free S3 Gateway Endpoint, artifact S3, ECR/prerequisites, IAM, SNS/Budget, application Secret container and small log groups, while planning removal of RDS, ALB, seven paid Interface Endpoints, ECS/Cloud Map/task definitions and infrastructure alarms. The estimated idle floor is $2–4/month versus about $210/month active fixed cost. See `docs/staging_idle_mode.md`. IDLE is a reviewed mode plan, never `terraform destroy`; RDS removal remains blocked until a manual snapshot, restore evidence, deletion-protection plan and human approval exist.

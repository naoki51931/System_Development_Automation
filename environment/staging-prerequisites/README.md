...
This independent Terraform root owns only the two staging ECR repositories, the GitHub staging deploy role and policy, the staging SNS topic/email subscription, and the 100 GBP staging Budget. Its state key is `system-navigator/staging/prerequisites.tfstate`. Custom-domain ACM and DNS validation are conditional and disabled by default; no domain is selected.

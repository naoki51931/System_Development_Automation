...
- The 2026-08-12 PITR-remediated plan was partially applied: ECS downsizing, autoscaling, SNS and seven ALB/ECS alarms succeeded, but AWS rejected Multi-AZ `db.t4g.small` with `InsufficientDBInstanceCapacity`. RDS remains medium with no pending modification and six RDS alarms are absent. Do not reapply the consumed saved plan; a fresh residual plan and separate approval are required after capacity is available.
- The review-only residual plan `production-100rpm-residual.tfplan` has SHA-256 `992ec5ffb8ec5da4383b29c23e908689db7f4339bb4eccc71f18f0dc3bbc5578`: 6 RDS alarms plus one in-place medium-to-small RDS change, with zero replace/destroy/ECS/Staging action. `db.t4g.small` is orderable for PostgreSQL 18.3 Multi-AZ gp2 in eu-west-2a/b/c, but instantaneous capacity is not guaranteed. Never apply without separate checksum-specific approval.
- The residual plan was applied once and is now consumed: six RDS alarms exist and the small class is pending the Saturday maintenance window. Never reapply it. After maintenance, verify `db.t4g.small`, PITR and alarm health, then require a fresh read-only no-op plan. SNS email confirmation remains manual.
...
- Terraform defines but never runs the migration task. `enable_release_runtime=true` is hard-failed by lifecycle preconditions unless a verifier-produced signed canonical schema-v1 attestation matches the fixed Production cluster/task/task-definition, approved post-merge release SHA, exact application digest, exit zero, fresh timestamp, and Alembic head `8d4f2a7c9b11`. `scripts/verify_production_migration_attestation.py` uses read-only ECS APIs, recomputes the canonical checksum, and verifies Ed25519 origin; Production DB head verification remains a separately approved step.

...
- `environment/staging-prerequisites` owns the two immutable ECR repositories, GitHub staging deploy role/policy, staging SNS email subscription, and 100 GBP Budget in state `system-navigator/staging/prerequisites.tfstate`. Custom-domain ACM and DNS validation are conditional and disabled until a new domain is explicitly approved. It owns no VPC, RDS, ECS, ALB, S3, or Secrets.

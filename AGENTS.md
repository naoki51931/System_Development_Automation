...
- Terraform defines but never runs the migration task. `enable_release_runtime=true` is rejected unless `migration_succeeded_app_image_uri` exactly equals `app_image_uri`; the attestation follows a separately approved task run, exit-zero check, and Alembic-head verification.
...
- Production worker uses the implemented `python -m app.workers.runner` entry point at 256 CPU/512 MiB and desired count one. Running-task, CPU, heartbeat-age, and dead-letter alarms are created only with release runtime.

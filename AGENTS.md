...
- Production monitoring is isolated on the planned `ai-platform-prod-alerts` topic with 13 ALB/ECS/RDS alarms; it never uses staging SNS. An empty `production_notification_email` permits review planning but creates no subscription, so Production apply remains blocked until a human selects the recipient and later confirms the AWS email subscription.

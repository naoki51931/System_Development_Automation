...
- The approved staging operations recipient is `info@nagi-neco.com`, supplied only through ignored `environment/staging/terraform.tfvars`. CloudWatch alarms publish to `system-navigator-staging-alerts`; its email subscription remains `PendingConfirmation` until a human confirms it. AWS Budgets sends 50/80/100% actual and 100% forecast notifications directly to the same address for the 100 GBP monthly budget. Never auto-confirm, send application mail to this address, build SES inbound/MX/S3 inbox infrastructure, or alter `nagi-neco.com` DNS/mail service.

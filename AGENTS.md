...
- Disabling Staging RDS deletion protection is a distinct ACTIVE-mode phase gated by `allow_database_deletion=false` by default, the existing removal approval, and an AWS-verified available manual snapshot. Its saved plan must change only `deletion_protection`; never combine it with the IDLE destroy plan.

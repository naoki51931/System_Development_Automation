...
- Terraform defines but never runs the migration task. `enable_release_runtime=true` is hard-failed by lifecycle preconditions unless a verifier-produced structured attestation matches the fixed Production cluster/task/task-definition, exact application digest, exit zero, and Alembic head `8d4f2a7c9b11`. `scripts/verify_production_migration_attestation.py` uses read-only ECS APIs; Production DB head verification remains a separately approved step.

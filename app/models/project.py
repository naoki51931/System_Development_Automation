...
class Project(TimestampMixin, Base):
    ...
    current_phase: Mapped[str] = mapped_column(
        String(30), nullable=False, default="hearing"
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            "organization_id", "project_code", name="uq_projects_organization_code"
        ),
        CheckConstraint(
            f"status IN ({sql_values(PROJECT_STATUSES)})", name="ck_projects_status"
        ),
        CheckConstraint(
            f"current_phase IN ({sql_values(PROJECT_PHASES)})", name="ck_projects_phase"
        ),
...
class ProjectMember(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            f"project_role IN ({sql_values(PROJECT_ROLES)})",
            name="ck_project_members_role",
        ),
        CheckConstraint(
            "status IN ('active', 'inactive')", name="ck_project_members_status"
        ),
...
class AIRun(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            f"provider IN ({sql_values(AI_PROVIDERS)})", name="ck_ai_runs_provider"
        ),
        CheckConstraint(
        ...
        CheckConstraint(
            "estimated_cost IS NULL OR estimated_cost >= 0", name="ck_ai_runs_cost"
        ),
        CheckConstraint(
            "billed_minutes IS NULL OR billed_minutes >= 0",
            name="ck_ai_runs_billed_minutes",
        ),
        CheckConstraint(
            "calculated_cost IS NULL OR calculated_cost >= 0",
            name="ck_ai_runs_calculated_cost",
        ),
...
class Artifact(TimestampMixin, Base):
    ...
    versions: Mapped[list["ArtifactVersion"]] = relationship(
        back_populates="artifact", foreign_keys="ArtifactVersion.artifact_id"
    )
    __table_args__ = (
        CheckConstraint(
            f"artifact_type IN ({sql_values(ARTIFACT_TYPES)})", name="ck_artifacts_type"
        ),
        CheckConstraint(
            f"status IN ({sql_values(ARTIFACT_STATUSES)})", name="ck_artifacts_status"
        ),
...
class ArtifactVersion(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "artifact_id", "version_number", name="uq_artifact_versions_number"
        ),
        ...
        CheckConstraint(
            "generated_by IN ('human', 'ai')", name="ck_artifact_versions_generated_by"
        ),
        CheckConstraint(
            "(generated_by = 'ai' AND ai_run_id IS NOT NULL) OR (generated_by = 'human' AND ai_run_id IS NULL)",
            name="ck_artifact_versions_ai_run",
        ),
...
class Review(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            f"review_type IN ({sql_values(REVIEW_TYPES)})", name="ck_reviews_type"
        ),
        CheckConstraint(
            f"status IN ({sql_values(REVIEW_STATUSES)})", name="ck_reviews_status"
        ),
        CheckConstraint(
            "score IS NULL OR (score >= 0 AND score <= 100)", name="ck_reviews_score"
        ),
        CheckConstraint(
            "(review_type = 'ai' AND ai_run_id IS NOT NULL AND reviewer_user_id IS NULL) OR (review_type <> 'ai' AND reviewer_user_id IS NOT NULL AND ai_run_id IS NULL)",
            name="ck_reviews_reviewer_source",
        ),
...
class ReviewComment(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "target_line IS NULL OR target_line > 0",
            name="ck_review_comments_target_line",
        ),
...
class ApprovalEvent(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "action IN ('submitted','approved','rejected','changes_requested','resubmitted','customer_accepted')",
            name="ck_approval_events_action",
        ),
        Index(
            "ix_approval_events_version_created", "artifact_version_id", "created_at"
        ),

...
def get_email_provider(request: Request) -> EmailProvider:
    ...
    if provider is None:
        outcomes = [
            value
            for value in os.getenv("MOCK_EMAIL_OUTCOMES", "delivered").split(",")
            if value
        ]
...
def preference_json(item: NotificationPreference) -> dict[str, object]:
    return {
        ...
        "quiet_hours_end": item.quiet_hours_end.isoformat()
        if item.quiet_hours_end
        else None,
...
def change_request_json(item: ChangeRequest, session: Session) -> dict[str, object]:
    impacts = session.scalars(
        select(ChangeRequestImpact).where(
            ChangeRequestImpact.change_request_id == item.id
        )
    ).all()
    return {
        "id": str(item.id),
        "project_id": str(item.project_id),
        "chat_message_id": str(item.chat_message_id),
        "title": item.title,
        "description": item.description,
        "status": item.status,
        "priority": item.priority,
        "scope_type": item.scope_type,
        "estimate_id": str(item.estimate_id) if item.estimate_id else None,
        "version": item.version,
        "impacts": [
            {
                ...
                "estimated_amount": str(value.estimated_amount),
            }
...
def list_notifications(
    ...
    statement = select(Notification).where(
        ...
        Notification.user_id == authenticated.user.id,
        Notification.status != "dismissed",
    )
    return paginate_query(
        session, statement, Notification, cursor, page_size, notification_json
    )
...
def list_preferences(
    ...
    values = session.scalars(
        ...
        .where(
            ...
            NotificationPreference.user_id == authenticated.user.id,
        )
...
@router.post(
    "/projects/{project_id}/documents/generate", status_code=status.HTTP_201_CREATED
)
def post_document(
    project_id: uuid.UUID,
    payload: DocumentGenerateInput,
    authenticated: Annotated[AuthenticatedUser, Depends(get_current_user)],
    session: Annotated[Session, Depends(get_session)],
    ...
    return {
        ...
        "artifact_version_id": str(job.artifact_version_id),
...
def document_download_url(
    ...
    job = session.scalar(
        select(DocumentGenerationJob.id).where(
            DocumentGenerationJob.artifact_version_id == version.id,
...
def send_document_email(
    ...
    if artifact.status != "approved" or artifact.current_version_id != version.id:
        raise AppError(
            "DOCUMENT_SEND_FORBIDDEN", "Only the approved current document can be sent"
        )
    ...
    require_organization_access(
        artifact.organization_id, AuthenticatedUser(recipient), session
    )
    template = resolve_email_template(
        session, artifact.organization_id, payload.template_code
    )
    download_url = storage.create_download_url(
        version.storage_key, payload.expires_seconds
    )
    notification = create_notification(
        ...
        body=f"{artifact.title} is ready.",
        severity="info",
        action_url=f"/artifacts/{artifact.id}",
        deduplication_key=f"document-email:{payload.deduplication_key}",
    )
    ...
    message = create_email_message(
        ...
        values={
            ...
            "expires_at": str(payload.expires_seconds),
        },
...
def list_rooms(
    ...
    rooms = session.scalars(
        select(ChatRoom)
        .where(
            ChatRoom.organization_id == project.organization_id,
...
def room_context(
    session: Session, room_id: uuid.UUID, authenticated: AuthenticatedUser
):
...
def patch_message(
    ...
    access = require_organization_access(
        message.organization_id, authenticated, session
    )
...
@router.post(
    "/chat-messages/{message_id}/attachments", status_code=status.HTTP_201_CREATED
)
def post_attachment(
    message_id: uuid.UUID,
    ...
    attachment = attach_chat_artifact(
        session, storage, message, version, artifact, access, payload.filename
    )
    session.commit()
    return {
...
@router.post(
    "/chat-messages/{message_id}/change-requests", status_code=status.HTTP_201_CREATED
)

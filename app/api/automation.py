...
from app.services.storage import ALLOWED_MIME_TYPES, DEFAULT_MAX_FILE_SIZE, ArtifactStorage, LocalArtifactStorage
...
class AISettingCreate(BaseModel):
    ...
    minute_rate: Decimal = Field(default=Decimal("0"), ge=0, max_digits=18, decimal_places=8)
    token_input_rate: Decimal = Field(default=Decimal("0"), ge=0, max_digits=18, decimal_places=8)
    token_output_rate: Decimal = Field(default=Decimal("0"), ge=0, max_digits=18, decimal_places=8)
...
class AISettingUpdate(BaseModel):
    ...
    minute_rate: Decimal | None = Field(default=None, ge=0, max_digits=18, decimal_places=8)
    token_input_rate: Decimal | None = Field(default=None, ge=0, max_digits=18, decimal_places=8)
    token_output_rate: Decimal | None = Field(default=None, ge=0, max_digits=18, decimal_places=8)
...
def setting_json(setting: AISetting | ResolvedAISetting) -> dict[str, object]:
    return {
        ...
        "organization_id": str(setting.organization_id) if isinstance(setting, AISetting) else None,
        "project_id": str(setting.project_id) if isinstance(setting, AISetting) and setting.project_id else None,
        ...
        "source": setting.source if isinstance(setting, ResolvedAISetting) else ("project" if setting.project_id else "organization"),
    }


@router.post("/ai-settings", status_code=status.HTTP_201_CREATED)
def post_ai_setting(payload: AISettingCreate, authenticated: Annotated[AuthenticatedUser, Depends(get_current_user)], session: Annotated[Session, Depends(get_session)]):
    require_organization_access(payload.organization_id, authenticated, session, PROJECT_WRITE_ROLES)
    if payload.project_id is not None:
        project = session.get(Project, payload.project_id)
        if project is None:
            raise AppError("RESOURCE_NOT_FOUND", "Project not found")
        if project.organization_id != payload.organization_id:
            raise AppError("PERMISSION_DENIED", "Project belongs to another organization")
...
def get_resolved_ai_setting(
    ...
    return setting_json(resolve_ai_setting(session, organization_id, project_id, provider, model, operation_type))


@router.patch("/ai-settings/{setting_id}")
def patch_ai_setting(setting_id: uuid.UUID, payload: AISettingUpdate, authenticated: Annotated[AuthenticatedUser, Depends(get_current_user)], session: Annotated[Session, Depends(get_session)]):
    setting = session.get(AISetting, setting_id)
    if setting is None:
        raise AppError("RESOURCE_NOT_FOUND", "AI setting not found")
    require_organization_access(setting.organization_id, authenticated, session, PROJECT_WRITE_ROLES)
...
@router.post("/artifacts/{artifact_id}/upload-intents", status_code=status.HTTP_201_CREATED)
def post_upload_intent(
    ...
    intent, upload_url = create_upload_intent(session, storage, artifact, access, **payload.model_dump())
    session.commit()
    return {"version_id": str(intent.version_id), "upload_url": upload_url, "expires_seconds": payload.expires_seconds}
...
def post_complete_upload(
    ...
    intent = session.scalar(select(ArtifactUploadIntent).where(ArtifactUploadIntent.version_id == version_id))
    if intent is None:
    ...
    session.commit()
    return {"id": str(version.id), "version_number": version.version_number, "content_hash": version.content_hash}
...
def _comment_action(comment_id: uuid.UUID, action: str, payload: CommentActionRequest, authenticated: AuthenticatedUser, session: Session):
    ...
    transition_comment(session, comment, review, access, action, uuid.uuid4(), payload.version)
    session.commit()
    return {"id": str(comment.id), "status": comment.status, "review_version": review.version}


@router.post("/review-comments/{comment_id}/accept")
def accept_comment(comment_id: uuid.UUID, payload: CommentActionRequest, authenticated: Annotated[AuthenticatedUser, Depends(get_current_user)], session: Annotated[Session, Depends(get_session)]):
    return _comment_action(comment_id, "accepted", payload, authenticated, session)


@router.post("/review-comments/{comment_id}/reject")
def reject_comment(comment_id: uuid.UUID, payload: CommentActionRequest, authenticated: Annotated[AuthenticatedUser, Depends(get_current_user)], session: Annotated[Session, Depends(get_session)]):
    return _comment_action(comment_id, "rejected", payload, authenticated, session)


@router.post("/review-comments/{comment_id}/resolve")
def resolve_comment(comment_id: uuid.UUID, payload: CommentActionRequest, authenticated: Annotated[AuthenticatedUser, Depends(get_current_user)], session: Annotated[Session, Depends(get_session)]):

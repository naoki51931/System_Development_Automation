...
SECURITY_EVENT_TYPES = frozenset(
    {"payment_failed", "maintenance_suspended", "deletion_scheduled"}
)
...
DANGEROUS_TEMPLATE_PATTERN = re.compile(
    r"{[%#]|__|\b(import|eval|exec|open|globals|class)\b", re.IGNORECASE
)
HTML_TAG_PATTERN = re.compile(r"<[^>]+>")
SCRIPT_BLOCK_PATTERN = re.compile(
    r"<(script|style|iframe|object|embed)[^>]*>.*?</\1\s*>", re.IGNORECASE | re.DOTALL
)
EXTERNAL_REFERENCE_PATTERN = re.compile(
    r"(?i)(https?://|file:|javascript:|data:text/html|src\s*=|url\s*\()"
)
...
PRIVILEGED_PROJECT_ROLES = frozenset(
    {"organization_owner", "organization_admin", "project_manager", "reviewer"}
)
...
def validate_internal_url(value: str | None) -> str | None:
    ...
    if (
        ...
        or value.startswith("//")
        or "\\" in value
        or re.search(r"(?i)javascript:|https?://", value)
    ):
...
def scheduled_delivery_time(
    preference: NotificationPreference | None, now: datetime, severity: str
) -> datetime | None:
...
def update_preference(
    ...
    if preference.user_id != access.user.id and access.role_codes.isdisjoint(
...
def create_notification(
    ...
    *,
    ...
    notification = Notification(
        ...
        severity=severity,
        status="pending",
        action_url=validate_internal_url(action_url),
    ...
    if severity == "critical":
        record_audit_log(
            ...
            organization_id=organization_id,
            actor_user_id=None,
            action="notification.critical_created",
...
def deliver_notification(
    ...
    try:
        result = provider.send(
            recipient_reference=str(notification.user_id),
...
def resolve_email_template(
    session: Session, organization_id: uuid.UUID, template_code: str, locale: str = "ja"
) -> EmailTemplate:
...
def render_template(
    source: str, values: dict[str, str], *, html_output: bool = False
) -> str:
    if DANGEROUS_TEMPLATE_PATTERN.search(source):
        raise AppError(
            "INVALID_TEMPLATE_DATA", "Template contains unsupported expressions"
        )
    referenced = set(VARIABLE_PATTERN.findall(source))
    if (
        referenced - ALLOWED_TEMPLATE_VARIABLES
        or set(values) - ALLOWED_TEMPLATE_VARIABLES
    ):
    ...
    escaped = {
        key: html.escape(sanitize_sensitive(str(value)))
        for key, value in values.items()
    }
    try:
        rendered = VARIABLE_PATTERN.sub(
            lambda match: escaped.get(match.group(1), "未設定"), source
        )
    ...
    return (
        ...
        if html_output
        else html.unescape(HTML_TAG_PATTERN.sub("", rendered))
    )
...
def create_email_message(
    ...
    *,
    ...
    if "\r" in subject or "\n" in subject:
        raise AppError(
            "INVALID_TEMPLATE_DATA", "Email subject contains invalid characters"
        )
    body_text = render_template(template.body_text_template, values)
    body_html = (
        render_template(template.body_html_template, values, html_output=True)
...
def send_email_message(
    session: Session, message: EmailMessage, provider: EmailProvider, now: datetime
) -> EmailMessage:
    ...
    try:
        result = provider.send_email(
            recipient=message.recipient_email_snapshot,
            subject=message.subject_snapshot,
            body_text=message.body_text_snapshot,
            body_html=message.body_html_snapshot,
        )
...
def _estimate_document(
    session: Session, estimate: Estimate, project: Project, organization: Organization
) -> str:
    ...
    lines = [
        "SystemNavigator AI - Estimate",
        f"Estimate number: {estimate.estimate_number}",
        f"Customer organization: {organization.name}",
        f"Project: {project.name}",
        f"Created: {estimate.created_at.date() if estimate.created_at else '未設定'}",
        f"Valid until: {estimate.valid_until}",
        "Items:",
    ]
    lines += [
    ...
    lines += [
        f"Subtotal: {estimate.subtotal}",
        f"Tax: {estimate.tax_amount}",
        f"Total: {estimate.total_amount}",
...
def _contract_document(
    session: Session, contract: Contract, project: Project, organization: Organization
) -> str:
    ...
    return "\n".join(
        [
            "SystemNavigator AI - Contract",
            f"Contract number: {contract.contract_number}",
            f"Contract type: {contract.contract_type}",
            f"Customer organization: {organization.name}",
            f"Project: {project.name}",
            f"Estimate number: {estimate.estimate_number}",
            f"Contract amount: {estimate.total_amount} {estimate.currency}",
            f"Terms version: {contract.terms_version}",
...
def _basic_design_document(project: Project) -> str:
    return "\n".join(
        [
            "SystemNavigator AI - Basic Design",
            f"Project: {project.name}",
            f"Overview: {project.description or '未設定'}",
            "Purpose: 未設定",
            "Target users: 未設定",
            "System architecture: 未設定",
            "Main screens: 未設定",
            "Main APIs: 未設定",
            "Main tables: 未設定",
            "AI review policy: 未設定",
            "Human review policy: 未設定",
            "Non-functional requirements: 未設定",
            "Revision history: 未設定",
        ]
    )
...
def generate_document(
    ...
    *,
    ...
    payload_hash = hashlib.sha256(
        json.dumps(authoritative, sort_keys=True).encode()
    ).hexdigest()
    existing = session.scalar(
    ...
    if document_type == "estimate":
        ...
        if (
            estimate is None
            or estimate.organization_id != project.organization_id
        ...
        if (
            contract is None
            or contract.organization_id != project.organization_id
    ...
    extension = (
        "pdf" if output_format == "pdf" else "html" if output_format == "html" else "md"
    )
    ...
    content = (
        ...
        if output_format == "pdf"
            ...
            f"<html><body><pre>{html.escape(text)}</pre></body></html>".encode()
    ...
    version = ArtifactVersion(
        ...
        generated_by="human",
        created_by_user_id=access.user.id,
        change_summary="Locally generated document",
    )
    session.add(version)
    session.flush()
    ...
    emit_project_event(
        ...
        body=f"{document_type} document was generated.",
        severity="success",
        action_url=f"/artifacts/{artifact.id}",
        actor_user_id=access.user.id,
        aggregate_type="document_generation_job",
        aggregate_id=job.id,
    )
    return job


def require_project_chat_access(
    session: Session, project: Project, access: OrganizationAccess
) -> None:
    ensure_resource_organization(access, project.organization_id)
...
def create_chat_room(
    ...
    *,
    ...
    room = ChatRoom(
        ...
        project_id=project.id,
        room_type=room_type,
        name=sanitize_plain_text(name, maximum=255),
        status="active",
        created_by_user_id=access.user.id,
    )
    session.add(room)
...
def post_chat_message(
    ...
    *,
    ...
    if message_type in {"ai", "system"}:
        raise AppError(
            "CHAT_ACCESS_FORBIDDEN", "Human users cannot post as AI or system"
        )
    ...
    emit_project_event(
        ...
        body="A new project chat message was posted.",
        severity="info",
        action_url=f"/chat-rooms/{room.id}",
        actor_user_id=access.user.id,
...
def delete_chat_message(
    ...
    ensure_resource_organization(access, message.organization_id)
    if message.sender_user_id != access.user.id and access.role_codes.isdisjoint(
        PRIVILEGED_PROJECT_ROLES
    ):
...
def attach_chat_artifact(
    ...
    ensure_resource_organization(access, message.organization_id)
    if (
        artifact.organization_id != message.organization_id
        or artifact.project_id != message.project_id
    ...
    if (
        ...
        or metadata.size != version.file_size
        or metadata.mime_type != version.mime_type
    ):
...
def create_change_request(
    ...
    *,
    ...
    if estimate_id:
        ...
        if (
            estimate is None
            or estimate.organization_id != message.organization_id
            or estimate.project_id != message.project_id
        ):
    ...
    request = ChangeRequest(
        ...
        title=sanitize_plain_text(title, maximum=255),
    ...
    if project is not None:
        emit_project_event(
            ...
            body=f"Change request {request.title} was submitted.",
            severity="info",
            action_url=f"/change-requests/{request.id}",
            actor_user_id=access.user.id,
...
def analyze_change_request(
    ...
    impact_type = {
        ...
        "major_change": "schedule",
        "new_feature": "cost",
    }.get(request.scope_type, "document")
    session.add(
        ChangeRequestImpact(
            change_request_id=request.id,
            impact_type=impact_type,
            target_reference=None,
            description=f"Mock AI analysis; score={result.score if result.score is not None else '未設定'}. Human verification required.",
            estimated_minutes=max(30, result.input_tokens * 2),
...
def decide_change_request(
    ...
    ensure_resource_organization(access, request.organization_id)
    check_version(request, expected_version)
    if "customer" not in access.role_codes and access.role_codes.isdisjoint(
    ...
    if request.status != "awaiting_customer_approval":
        raise AppError(
            "INVALID_STATE_TRANSITION", "Change request is not awaiting approval"
        )
    if action == "approve":
        impacts = session.scalar(
            select(ChangeRequestImpact.id).where(
                ChangeRequestImpact.change_request_id == request.id
            )
        )
        if (
            request.scope_type in {"major_change", "new_feature", "infrastructure"}
            and impacts is None
        ):
            raise AppError(
                "INVALID_STATE_TRANSITION", "Human review of major impact is required"
            )
    ...
    if request.status == "approved":
        ...
        if project is not None:
            emit_project_event(
                ...
                body=f"Change request {request.title} was approved.",
                severity="success",
                action_url=f"/change-requests/{request.id}",
                actor_user_id=access.user.id,
                aggregate_type="change_request",
...
def process_outbox_event(
    session: Session, event: OutboxEvent, handler, now: datetime
) -> OutboxEvent:
    ...
    try:
        ...
        event.available_at = now + timedelta(
            seconds=min(3600, 2**event.attempt_count * 60)
        )
    return event
...
def emit_project_event(
    ...
    *,
    ...
    payload = {
        "title": sanitize_sensitive(title),
        "body": sanitize_sensitive(body),
        "severity": severity,
    ...
    recipients.update(
        session.scalars(
            select(ProjectMember.user_id).where(
                ProjectMember.project_id == project.id, ProjectMember.status == "active"
            )
        ).all()
    )
...
def resolve_document_template(
    session: Session, organization_id: uuid.UUID, document_type: str, locale: str = "ja"
) -> DocumentTemplate:
    template = session.scalar(
...
def validate_document_payload(schema: dict, payload: dict) -> None:
    if schema.get("type") != "object" or not isinstance(payload, dict):
        raise AppError(
            "INVALID_TEMPLATE_DATA", "Document payload does not match schema"
        )
    ...
    if schema.get("additionalProperties", False) is False and set(payload) - set(
        properties
    ):
    ...
    for key, value in payload.items():
        ...
        if isinstance(value, str) and EXTERNAL_REFERENCE_PATTERN.search(value):
            raise AppError(
                "INVALID_TEMPLATE_DATA", "External document references are forbidden"
            )

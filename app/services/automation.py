...
def resolve_ai_setting(
    ...
    conditions = [
        AISetting.organization_id == organization_id,
        AISetting.provider == provider,
        AISetting.model == model,
        AISetting.operation_type == operation_type,
    ]
    setting = None
    if project_id is not None:
        setting = session.scalar(
            select(AISetting).where(*conditions, AISetting.project_id == project_id)
        )
    source = "project"
    if setting is None:
        setting = session.scalar(
            select(AISetting).where(*conditions, AISetting.project_id.is_(None))
        )
        source = "organization"
    if setting is None:
    ...
    return ResolvedAISetting(
        ...
        setting.enabled,
        setting.review_threshold,
        setting.max_auto_revision_count,
        setting.minute_rate,
        setting.token_input_rate,
        setting.token_output_rate,
        setting.currency,
        source,
        setting.version,
    )
...
def calculate_ai_cost(
    ...
    cost = (
        Decimal(billed_minutes) * minute_rate
...
def apply_ai_cost(run: AIRun, setting: ResolvedAISetting) -> Decimal:
    minutes, cost = calculate_ai_cost(
...
def update_ai_setting(
    session: Session, setting: AISetting, expected_version: int, values: dict
) -> AISetting:
...
def create_upload_intent(
    ...
    *,
...
) -> tuple[ArtifactUploadIntent, str]:
    ...
    key = storage.build_storage_key(
        artifact.organization_id, artifact.project_id, artifact.id, version_id, filename
    )  # type: ignore[attr-defined]
    if len(content_hash) != 64 or any(
        c not in "0123456789abcdef" for c in content_hash.lower()
    ):
    ...
    intent = ArtifactUploadIntent(
        ...
        expected_hash=content_hash.lower(),
...
def complete_upload(
    ...
    if (
        ...
        or artifact.organization_id != intent.organization_id
    ):
...
def transition_comment(
    ...
    if (comment.status, action) not in allowed:
        raise AppError(
            "INVALID_STATE_TRANSITION", "Comment state transition is not allowed"
        )
    ...
    record_audit_log(
        ...
        action=f"review_comment.{action}",
...
def run_auto_revision(
    ...
    job = WorkflowJob(
        ...
        status="running",
        attempt_count=0,
        max_attempts=setting.max_auto_revision_count,
        locked_at=datetime.now(timezone.utc),
        locked_by="local-worker",
    )
    session.add(job)
    session.flush()
    ...
    for attempt in range(1, job.max_attempts + 1):
        ...
        run = create_ai_run(
            ...
            duration_ms=revised.duration_ms + reviewed.duration_ms,
        ...
        version = ArtifactVersion(
            ...
            generated_by="ai",
            ai_run_id=run.id,
            change_summary="Mock AI automatic revision",
        )
        session.add(version)
        session.flush()
        artifact.current_version_id = version.id
        ...
        create_review(
            ...
            version,
            access,
            passing_score=setting.review_threshold,
            review_type="ai",
            ai_run_id=run.id,
            status="passed"
        ...
        if (reviewed.score or 0) >= setting.review_threshold:
            move_to_human_review(
                session, artifact, version, passing_score=setting.review_threshold
            )

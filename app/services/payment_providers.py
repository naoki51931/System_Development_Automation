...
class PaymentDeclined(PaymentProviderError):
    def __init__(
        self, code: str = "card_declined", message: str = "Payment was declined"
    ):
        self.code = code
...
class PaymentProvider(ABC):
    ...
    def create_subscription(
        self, customer_id: str, price_reference: str
...
class MockPaymentProvider(PaymentProvider):
    def __init__(
        self, outcomes: list[str] | None = None, webhook_secret: str | None = None
    ):
        self.outcomes = list(outcomes or ["succeeded"])
        self.webhook_secret = (
            webhook_secret.encode() if webhook_secret else secrets.token_bytes(32)
        )
    ...
    def create_customer(self, organization_reference: str) -> ProviderCustomer:
        return ProviderCustomer(
            f"mock_cus_{hashlib.sha256(organization_reference.encode()).hexdigest()[:20]}"
        )
    ...
    def attach_payment_method(
        ...
        if not payment_method_token.startswith("mock_pm_"):
            raise PaymentDeclined(
                "invalid_payment_method", "Payment method token is invalid"
            )
        return ProviderPaymentMethod(
            f"mock_method_{uuid.uuid4().hex}", "visa", "4242", 12, 2030
        )
    ...
    def create_payment_intent(
        ...
        result = ProviderPaymentIntent(
            identifier, "requires_confirmation", amount, currency
        )
...
class StripePaymentProviderStub(PaymentProvider):
    ...
    create_customer = attach_payment_method = create_payment_intent = (
        confirm_payment
    ) = _disabled
    create_subscription = cancel_subscription = retry_invoice = get_payment_status = (
        _disabled
    )

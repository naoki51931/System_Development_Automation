...
class AIProviderRateLimited(AIProviderError): pass
...
class DisabledExternalProvider(AIProvider):
    def _disabled(self): raise AIProviderUnavailable("External AI communication is disabled")
    def generate(self, content: bytes, instructions: list[str]) -> AIResult: return self._disabled()
    def review(self, content: bytes) -> AIResult: return self._disabled()
    def revise(self, content: bytes, comments: list[str]) -> AIResult: return self._disabled()
    def test_connection(self) -> bool: return False


class OpenAIProviderStub(DisabledExternalProvider): pass
class AnthropicProviderStub(DisabledExternalProvider): pass

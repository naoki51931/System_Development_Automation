...
SECRET_PATTERN = re.compile(
    r"(?i)(card|cvc|api[_-]?key|token|secret|password)\s*[:=]\s*\S+"
)
...
def require_roles(
    ...
    code: str = "PAYMENT_ACTION_FORBIDDEN",
) -> None:
...
def safe_flush(session: Session) -> None:
    try:
        ...
        raise AppError(
            "VERSION_CONFLICT", "Resource was updated by another request"
        ) from exc
...
def calculate_item_amount(
    item_type: str, quantity: Decimal, unit_price: Decimal
) -> Decimal:
    if quantity < 0:
    ...
    if item_type == "discount":
        if unit_price > 0:
            raise AppError(
                "INVALID_AMOUNT", "Discount unit price must be negative or zero"
            )
    elif unit_price < 0:
...
def calculate_tax(
    taxable_amount: Decimal, tax_rate: Decimal = DEFAULT_TAX_RATE
) -> Decimal:
...
def recalculate_estimate(
    session: Session, estimate: Estimate, tax_rate: Decimal = DEFAULT_TAX_RATE
) -> Estimate:
    ...
    subtotal = money(
        sum((item.amount for item in items if item.item_type != "tax"), Decimal("0"))
    )
    if subtotal < 0:
...
def add_estimate_item(
    ...
    *,
    ...
    order = (
        session.scalar(
            select(func.max(EstimateItem.display_order)).where(
...
def generate_ai_run_items(session: Session, estimate: Estimate) -> None:
    runs = session.scalars(
        select(AIRun).where(
            AIRun.organization_id == estimate.organization_id,
            AIRun.project_id == estimate.project_id,
            AIRun.status == "completed",
            AIRun.calculated_cost.is_not(None),
        )
    ).all()
    for run in runs:
        exists = session.scalar(
            select(EstimateItem.id).where(
                EstimateItem.estimate_id == estimate.id,
                EstimateItem.source_type == "ai_run",
                EstimateItem.source_reference_id == run.id,
            )
        )
        if exists is None:
            add_estimate_item(
                session,
                estimate,
                item_type="ai_runtime",
                description=f"AI runtime ({run.operation_type})",
                quantity=Decimal("1"),
                unit="run",
                unit_price=money(run.calculated_cost or Decimal("0")),
                source_type="ai_run",
                source_reference_id=run.id,
            )


def resolve_artifact_pricing(
    session: Session, organization_id: uuid.UUID, artifact_type: str
) -> tuple[PricingRule | None, Decimal, Decimal, Decimal]:
    rule = session.scalar(
        select(PricingRule).where(
            PricingRule.organization_id == organization_id,
            PricingRule.artifact_type == artifact_type,
    ...
    if rule is None:
        rule = session.scalar(
            select(PricingRule).where(
                PricingRule.organization_id.is_(None),
                PricingRule.artifact_type == artifact_type,
                PricingRule.enabled.is_(True),
            )
        )
    if rule:
        return (
            ...
            rule.quality_multiplier,
        )
    return (
        None,
        DEFAULT_ARTIFACT_PRICES.get(artifact_type, Decimal("0")),
        Decimal("1"),
        Decimal("1"),
    )
...
def calculate_artifact_value(
    ...
    *,
    ...
    if (
        min(screen_count, api_count, table_count, test_case_count) < 0
        or min(base_price, complexity, quality) < 0
    ):
        raise AppError("INVALID_AMOUNT", "Artifact pricing inputs cannot be negative")
    size_factor = (
        Decimal("1")
        + Decimal(screen_count) * Decimal("0.01")
        + Decimal(api_count) * Decimal("0.02")
        + Decimal(table_count) * Decimal("0.015")
        + Decimal(test_case_count) * Decimal("0.001")
    )
...
def generate_artifact_value_items(session: Session, estimate: Estimate) -> None:
    artifacts = session.scalars(
        select(Artifact).where(
            Artifact.organization_id == estimate.organization_id,
            Artifact.project_id == estimate.project_id,
        )
    ).all()
    for artifact in artifacts:
        exists = session.scalar(
            select(EstimateItem.id).where(
                EstimateItem.estimate_id == estimate.id,
                EstimateItem.source_type == "artifact",
                EstimateItem.source_reference_id == artifact.id,
            )
        )
        if exists is not None:
            continue
        rule, base, complexity, quality = resolve_artifact_pricing(
            session, estimate.organization_id, artifact.artifact_type
        )
        value = calculate_artifact_value(base, complexity, quality)
        item = add_estimate_item(
            session,
            estimate,
            item_type="artifact_value",
            description=f"Artifact value: {artifact.title}",
            quantity=Decimal("1"),
            unit="artifact",
            unit_price=value,
            source_type="artifact",
            source_reference_id=artifact.id,
        )
        session.add(
            ArtifactPricingSnapshot(
                ...
                complexity_multiplier=money(complexity),
                quality_multiplier=money(quality),
                screen_count=0,
                api_count=0,
                table_count=0,
                test_case_count=0,
...
def transition_estimate(
    ...
    if estimate.status == "approved":
        raise AppError("INVALID_ESTIMATE_STATE", "Approved estimates are immutable")
    transitions = {
        ("draft", "submit"): "customer_review",
        ("customer_review", "approve"): "approved",
        ("customer_review", "reject"): "rejected",
    }
    target = transitions.get((estimate.status, action))
    if target is None:
        raise AppError(
            "INVALID_ESTIMATE_STATE", "Estimate state transition is not allowed"
        )
    if target == "approved":
    ...
    if project is not None and action in {"submit", "approve"}:
        ...
        emit_project_event(
            ...
            if action == "submit"
            else "estimate_approved",
            title=f"Estimate {action}",
            body=f"Estimate {estimate.estimate_number} was {action}ed.",
            severity="success" if action == "approve" else "info",
            action_url=f"/estimates/{estimate.id}",
...
def create_contract(
    ...
    *,
    ...
    if (
        ...
        raise AppError(
            "INVALID_CONTRACT_STATE", "Estimate and project scope do not match"
        )
    if estimate.status != "approved":
        raise AppError(
            "INVALID_ESTIMATE_STATE", "Only approved estimates can become contracts"
        )
    if estimate.valid_until < datetime.now(timezone.utc).date():
        raise AppError("ESTIMATE_EXPIRED", "Estimate has expired")
    contract = Contract(
        ...
        document_artifact_version_id=document_artifact_version_id,
    )
...
def accept_contract(
    ...
    *,
    ...
    if party == "customer":
        ...
        if (
            contract.status not in {"draft", "awaiting_customer"}
            or contract.customer_accepted_at
        ):
            raise AppError(
                "INVALID_CONTRACT_STATE", "Customer acceptance is not allowed"
            )
        contract.customer_accepted_by_user_id = access.user.id
        contract.customer_accepted_at = now
        contract.status = (
            "active" if contract.provider_accepted_at else "awaiting_provider"
        )
    elif party == "provider":
        require_roles(access, WRITE_ROLES)
        if (
            contract.status not in {"draft", "awaiting_provider"}
            or contract.provider_accepted_at
        ):
            raise AppError(
                "INVALID_CONTRACT_STATE", "Provider acceptance is not allowed"
            )
        contract.provider_accepted_by_user_id = access.user.id
        contract.provider_accepted_at = now
        contract.status = (
            "active" if contract.customer_accepted_at else "awaiting_customer"
        )
    else:
    ...
    record_audit_log(
        ...
        actor_user_id=access.user.id,
        action=f"contract.{party}_accepted",
        resource_type="contract",
        resource_id=contract.id,
        request_id=request_id,
        before={"status": before},
        after={"status": contract.status, "terms_version": contract.terms_version},
    )
    safe_flush(session)
    if contract.status == "active":
        from app.services.communications import emit_project_event
        ...
        if project is not None:
            emit_project_event(
                ...
                body=f"Contract {contract.contract_number} is active.",
                severity="success",
...
def create_contract_payment_intent(
    session: Session,
    contract: Contract,
    access: OrganizationAccess,
    ...
    *,
    ...
    if estimate is None or estimate.status != "approved":
        raise AppError(
            "INVALID_CONTRACT_STATE", "Approved contract estimate is unavailable"
        )
    if estimate.valid_until < datetime.now(timezone.utc).date():
        raise AppError(
            "ESTIMATE_EXPIRED", "Estimate was expired when payment was initiated"
        )
    if expected_amount is not None and money(expected_amount) != estimate.total_amount:
        raise AppError(
            "AMOUNT_MISMATCH", "Requested amount does not match contract amount"
        )
    existing_key = session.scalar(
        select(PaymentIntent).where(
            PaymentIntent.organization_id == contract.organization_id,
    ...
    duplicate = session.scalar(
        select(PaymentIntent.id).where(
            PaymentIntent.contract_id == contract.id,
    ...
    if duplicate is not None:
        raise AppError(
            "DUPLICATE_PAYMENT", "A payment already exists for this contract"
        )
    try:
        external = provider.create_payment_intent(
            None, estimate.total_amount, estimate.currency, idempotency_key
        )
    ...
    intent = PaymentIntent(
        ...
        provider="mock",
        provider_payment_intent_id=external.id,
...
def confirm_contract_payment(
    ...
    if (
        intent.contract_id != contract.id
        or intent.project_id != project.id
        or contract.organization_id != intent.organization_id
    ):
    ...
    emit_project_event(
        ...
        title=f"Payment {intent.status}",
        body=f"Payment for contract {contract.contract_number} is {intent.status}.",
        severity="success"
        if intent.status == "succeeded"
        else "error"
        if intent.status == "failed"
...
def create_maintenance_contract(
    ...
    if project.status not in {"production", "maintenance"}:
        raise AppError(
            "INVALID_CONTRACT_STATE", "Project is not eligible for maintenance"
        )
    if (
        contract.organization_id != project.organization_id
        or contract.project_id != project.id
        or contract.status != "active"
    ):
        raise AppError(
            "INVALID_CONTRACT_STATE", "Active maintenance contract scope is invalid"
        )
    if contract.contract_type != "maintenance":
        raise AppError(
            "INVALID_CONTRACT_STATE", "Contract is not a maintenance contract"
        )
    if (
        plan.organization_id not in {None, project.organization_id}
    ...
    record_maintenance_event(
        session, maintenance, None, "active", "contract_started", now
    )
    return maintenance
...
def mark_maintenance_payment_failed(
    session: Session, contract: MaintenanceContract, now: datetime
) -> MaintenanceContract:
    if contract.status not in {"active", "past_due"}:
        raise AppError(
            "INVALID_STATE_TRANSITION", "Maintenance payment failure cannot be applied"
        )
    old = contract.status
    ...
    if project is not None:
        ...
        emit_project_event(
            session,
            project,
            event_type="maintenance_past_due",
            title="Maintenance payment past due",
            body="Maintenance payment is past due.",
            severity="warning",
            action_url=f"/maintenance-contracts/{contract.id}",
            aggregate_type="maintenance_contract",
            aggregate_id=contract.id,
        )
    return contract


def advance_maintenance_delinquency(
    session: Session, contract: MaintenanceContract, now: datetime
) -> ResourceDeletionRequest | None:
    ...
    if elapsed >= timedelta(days=7) and elapsed < timedelta(days=30):
        retried = session.scalar(
            select(MaintenanceStatusEvent.id).where(
        ...
        if target == "suspended":
            ...
            if project is not None:
                ...
                emit_project_event(
                    session,
                    project,
                    event_type="maintenance_suspended",
                    title="Maintenance suspended",
                    body="Maintenance service is suspended.",
                    severity="critical",
                    action_url=f"/maintenance-contracts/{contract.id}",
                    aggregate_type="maintenance_contract",
                    aggregate_id=contract.id,
    ...
    existing = session.scalar(
        select(ResourceDeletionRequest).where(
            ResourceDeletionRequest.maintenance_contract_id == contract.id,
            ResourceDeletionRequest.status.notin_(["cancelled", "failed"]),
        )
    )
    if existing:
        return existing
    request = ResourceDeletionRequest(
        ...
        reason="Maintenance payment overdue for 90 days",
        scheduled_for=now,
    )
    session.add(request)
    session.flush()
...
def recover_maintenance_payment(
    session: Session, contract: MaintenanceContract, now: datetime
) -> MaintenanceContract:
    if contract.status not in {
        ...
        raise AppError(
            "INVALID_STATE_TRANSITION", "Maintenance contract cannot be restored"
        )
    old = contract.status
    ...
    requests = session.scalars(
        select(ResourceDeletionRequest).where(
            ResourceDeletionRequest.maintenance_contract_id == contract.id,
            ResourceDeletionRequest.status.in_(
                [
                    "draft",
...
def approve_deletion_request(
    ...
    if request.status == "pending_first_approval":
        ...
        if request.approved_by_first == access.user.id:
            raise AppError(
                "PAYMENT_ACTION_FORBIDDEN", "A second distinct approver is required"
            )
        ...
        raise AppError(
            "INVALID_STATE_TRANSITION", "Deletion request cannot be approved"
        )
    record_audit_log(
        session,
        organization_id=request.organization_id,
        actor_user_id=access.user.id,
        action="resource_deletion_request.approved",
        resource_type="resource_deletion_request",
...
def process_mock_webhook(
    ...
    existing = session.scalar(
        select(PaymentEvent).where(
            PaymentEvent.provider == "mock", PaymentEvent.provider_event_id == event_id
        )
    )
    if existing:
        raise AppError(
            "WEBHOOK_ALREADY_PROCESSED", "Webhook event was already processed"
        )
    now = datetime.now(timezone.utc)

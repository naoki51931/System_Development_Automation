...
from app.auth.dependencies import AuthenticatedUser, OrganizationAccess, ensure_resource_organization, require_organization_access
from app.models import OrganizationMembership
from app.models.project import (
    AIRun, ApprovalEvent, Artifact, ArtifactVersion, Project, ProjectMember, Review, ReviewComment,
)

AI_PASSING_SCORE_DEFAULT = 95
PROJECT_WRITE_ROLES = frozenset({"organization_owner", "organization_admin", "project_manager"})
...
def require_active_membership(session: Session, organization_id: uuid.UUID, user_id: uuid.UUID) -> None:
    exists = session.scalar(select(OrganizationMembership.id).where(
        OrganizationMembership.organization_id == organization_id,
        OrganizationMembership.user_id == user_id,
        OrganizationMembership.status == "active",
    ))
    if exists is None:
        raise domain_error("User is not an active organization member", status.HTTP_403_FORBIDDEN)
...
def create_project(session: Session, access: OrganizationAccess, **values) -> Project:  # type: ignore[no-untyped-def]
    ...
    for key in ("customer_user_id", "project_manager_user_id"):
        if values.get(key):
            require_active_membership(session, access.membership.organization_id, values[key])
...
def get_project_for_user(session: Session, project_id: uuid.UUID, authenticated: AuthenticatedUser) -> tuple[Project, OrganizationAccess]:
...
def add_project_member(session: Session, project: Project, user_id: uuid.UUID, project_role: str) -> ProjectMember:
    require_active_membership(session, project.organization_id, user_id)
    member = ProjectMember(project_id=project.id, user_id=user_id, project_role=project_role, status="active")
    session.add(member)
    session.flush()
    return member


def create_artifact(session: Session, project: Project, access: OrganizationAccess, **values) -> Artifact:  # type: ignore[no-untyped-def]
    ensure_resource_organization(access, project.organization_id)
    if access.role_codes.isdisjoint(ARTIFACT_WRITE_ROLES):
        raise domain_error("Permission denied", status.HTTP_403_FORBIDDEN)
    artifact = Artifact(organization_id=project.organization_id, project_id=project.id, **values)
    session.add(artifact)
    session.flush()
    return artifact


def add_artifact_version(session: Session, artifact: Artifact, access: OrganizationAccess, **values) -> ArtifactVersion:  # type: ignore[no-untyped-def]
    ...
    if values.get("generated_by") == "ai":
        ...
        if ai_run is None or ai_run.organization_id != artifact.organization_id or ai_run.project_id != artifact.project_id:
            raise domain_error("AI run does not belong to the artifact project", status.HTTP_403_FORBIDDEN)
    version = ArtifactVersion(artifact_id=artifact.id, version_number=next_number, **values)
    session.add(version)
    session.flush()
...
def create_review(session: Session, version: ArtifactVersion, access: OrganizationAccess, *, passing_score: int = AI_PASSING_SCORE_DEFAULT, **values) -> Review:  # type: ignore[no-untyped-def]
    ...
    if review_type == "ai":
        ...
        if values.get("status") == "passed" and (values.get("score") is None or values["score"] < passing_score):
        ...
        require_active_membership(session, artifact.organization_id, values["reviewer_user_id"])
    review = Review(organization_id=artifact.organization_id, project_id=artifact.project_id, artifact_version_id=version.id, **values)
    session.add(review)
    session.flush()
    return review


def add_review_comment(session: Session, review: Review, access: OrganizationAccess, **values) -> ReviewComment:  # type: ignore[no-untyped-def]
    ...
    if values.get("author_user_id") is not None:
        require_active_membership(session, review.organization_id, values["author_user_id"])
...
def _event(session: Session, artifact: Artifact, version: ArtifactVersion, actor_id: uuid.UUID, action: str, comment: str | None = None) -> None:
    session.add(ApprovalEvent(organization_id=artifact.organization_id, project_id=artifact.project_id, artifact_version_id=version.id, actor_user_id=actor_id, action=action, comment=comment))


def submit_version(session: Session, artifact: Artifact, version: ArtifactVersion, access: OrganizationAccess) -> None:
    ensure_resource_organization(access, artifact.organization_id)
    if access.role_codes.isdisjoint(ARTIFACT_WRITE_ROLES):
        raise domain_error("Permission denied", status.HTTP_403_FORBIDDEN)
    if artifact.current_version_id != version.id or artifact.status not in {"draft", "revision_requested"}:
...
def move_to_human_review(session: Session, artifact: Artifact, version: ArtifactVersion, *, passing_score: int = AI_PASSING_SCORE_DEFAULT) -> None:
    ...
    passed = session.scalar(select(Review.id).where(Review.artifact_version_id == version.id, Review.review_type == "ai", Review.status == "passed", Review.score >= passing_score))
    if passed is None:
        raise domain_error("Passing AI review is required")
    artifact.status = "human_reviewing"


def approve_version(session: Session, artifact: Artifact, version: ArtifactVersion, access: OrganizationAccess, comment: str | None = None) -> None:
    ensure_resource_organization(access, artifact.organization_id)
    if access.role_codes.isdisjoint(APPROVAL_ROLES):
        raise domain_error("Permission denied", status.HTTP_403_FORBIDDEN)
    if artifact.status != "human_reviewing" or artifact.current_version_id != version.id:
        raise domain_error("Artifact cannot be approved")
    critical = session.scalar(select(ReviewComment.id).join(Review).where(Review.artifact_version_id == version.id, ReviewComment.severity == "critical", ReviewComment.status.in_(["open", "accepted"])))
...
def create_ai_run(session: Session, *, estimated_cost: Decimal | None = None, error_message: str | None = None, **values) -> AIRun:  # type: ignore[no-untyped-def]
    run = AIRun(estimated_cost=estimated_cost, error_message_sanitized=sanitize_ai_error(error_message), **values)
    session.add(run)
    session.flush()
    return run

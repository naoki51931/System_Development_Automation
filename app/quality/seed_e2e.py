...
def seed(factory=None):
    ...
    with factory.begin() as session:
        ...
        owner_user=session.scalar(select(User).where(User.email=="organization_owner@quality.local"));owner_membership=session.scalar(select(OrganizationMembership).where(OrganizationMembership.organization_id==orgs[1].id,OrganizationMembership.user_id==owner_user.id)) or OrganizationMembership(organization_id=orgs[1].id,user_id=owner_user.id,status="active");session.add(owner_membership);session.flush()
        if not session.scalar(select(MembershipRole).where(MembershipRole.membership_id==owner_membership.id,MembershipRole.role_id==roles["organization_owner"].id)):session.add(MembershipRole(membership_id=owner_membership.id,role_id=roles["organization_owner"].id))
